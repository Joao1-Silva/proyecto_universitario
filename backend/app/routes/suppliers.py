...
def _serialize_supplier(model: SupplierModel, category_ids: list[str] | None = None) -> SupplierRead:
    ...
    if len(phone_number_digits) < 8:
        phone_number_digits = phone_number_digits.ljust(8, "0")
    phone_e164 = (model.phone_e164 or f"{phone_country_code}{phone_number_digits}").strip()
    if not phone_e164.startswith("+") or len(phone_e164) < 9:
        phone_e164 = f"{phone_country_code}{phone_number_digits}"
    return SupplierRead(
        id=model.id,
        name=model.name or "",
        rif=model.rif or "",
...
def create_supplier(
    ...
    existing_email = session.execute(
        select(SupplierModel).where(func.lower(SupplierModel.email) == payload.email.strip().lower())
    ).scalar_one_or_none()
    ...
    if existing_rif is not None:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="RIF already exists.")
...
def update_supplier(
    ...
    if "phoneNumber" in updates:
        ...
        supplier.phone_number = str(value).strip() if value is not None else None
    if "phoneE164" in updates:
        value = updates["phoneE164"]
        supplier.phone_e164 = str(value).strip() if value is not None else None
    if "phoneCountryCode" in updates or "phoneNumber" in updates or "phoneE164" in updates:
        supplier.phone = f"{supplier.phone_country_code or ''} {supplier.phone_number or ''}".strip()
